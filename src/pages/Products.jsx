        ...
        <div className="min-h-screen bg-brand-cream text-brand-black flex flex-col font-sans">
            ...
            <div className="w-full pt-32 md:pt-40 pb-16 md:pb-20 px-6 md:px-12 border-b ghost-border">
                <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
                    <span className="inline-block text-sm font-semibold text-brand-orange uppercase tracking-widest mb-4">Catalog</span>
                    <h1 className="text-5xl md:text-7xl font-black text-brand-black tracking-tight-brand mb-6">Systems Catalog</h1>
                    <p className="text-lg md:text-xl text-brand-black/50 max-w-2xl font-medium leading-relaxed">Practical tools, templates, and systems to streamline operations and scale faster.</p>
                </div>
            </div>
            <main className="flex-grow w-full max-w-7xl mx-auto px-6 md:px-12 py-16">
                <div className="flex flex-wrap gap-3 mb-12">
                    {CATEGORIES.map(category => (
                        <button key={category} onClick={() => setActiveTab(category)}
                            className={`px-5 py-2 rounded-full font-bold text-sm transition-all duration-300 ${activeTab === category ? 'bg-brand-orange text-white shadow-md' : 'bg-surface-container text-brand-black/60 hover:bg-surface-high hover:text-brand-black'}`}>
                            {category}
                        </button>
                    ))}
                </div>
                ...
                <div>
                    <h2 className="text-2xl font-black tracking-tight-brand mb-8 text-brand-black/40 flex items-center gap-3">
                        <span className="w-2 h-8 bg-brand-black/20 rounded-full" />Coming Soon
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {COMING_SOON_PRODUCTS.map(product => (
                            <div key={product.id} className="rounded-2xl border border-brand-black/5 bg-white/60 p-6 flex flex-col relative overflow-hidden opacity-60">
                                <span className="absolute top-4 right-4 bg-surface-container text-brand-black/40 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">Soon</span>
