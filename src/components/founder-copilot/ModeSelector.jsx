const ModeSelector = ({ modes = [], selectedMode, onSelect, compact = false }) => {
  return (
    ...
    >
      ...
      <h2 className={`${compact ? 'text-xl md:text-2xl' : 'text-2xl md:text-[2rem]'} font-black tracking-tight-brand mb-3`}>
        What do you need help with right now?
      </h2>
      <p className="text-sm md:text-base font-bold leading-relaxed text-brand-black/65 mb-5">
        ...
        {modes.map((mode) => {
          ...
          return (
            <button
              ...
              className={`text-left rounded-[20px] border-2 border-brand-black p-4 transition-all ${
                isActive
