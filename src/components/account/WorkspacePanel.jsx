function fieldInputClasses(multiline = false) {
  return `w-full rounded-2xl border border-brand-black/10 bg-brand-cream px-4 py-3 font-semibold outline-none focus:border-brand-orange ${
    ...
    <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div className="rounded-[24px] border border-brand-black/10 bg-white p-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <button type="button" onClick={onReset} className="rounded-full border border-brand-black/10 bg-brand-cream px-4 py-2 text-sm font-bold">
            New item
          </button>
        </div>
        <div className="mt-6 space-y-4">
          {memoryItems.length === 0 ? (
            ...
            <div key={item.id} className="rounded-2xl border border-brand-black/10 bg-brand-cream px-5 py-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => onEdit(item)} className="rounded-full border border-brand-black/10 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.12em]">Edit</button>
                  <button type="button" onClick={() => onArchive(item.id)} className="rounded-full border border-brand-black/10 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.12em]">Archive</button>
                </div>
              </div>
            </div>
          ))}
      ...
      <form onSubmit={onSave} className="rounded-[24px] border border-brand-black/10 bg-white p-6">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={onReset} className="rounded-2xl border border-brand-black/10 bg-white px-4 py-3 font-black uppercase tracking-[0.14em]">
              Reset
            </button>
          </div>
        </div>
