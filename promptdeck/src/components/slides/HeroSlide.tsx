...
interface Props {
  company_name: string; tagline: string; stage: string; year: number | string
...
export function HeroSlide({ company_name, tagline, stage, year, imageUrl, slideIndex }: Props) {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden flex flex-col items-center justify-center text-center">
      ...
      <div className="absolute inset-0" style={{
        background: imageUrl
          ? 'linear-gradient(135deg, rgba(10,10,30,0.95) 0%, rgba(10,10,30,0.88) 50%, rgba(26,10,62,0.82) 100%)'
          : 'linear-gradient(135deg, #0A0A0F 0%, #0f0a2e 50%, #1a0a3e 100%)'
      }} />
      {/* Grid overlay */}
      <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(124,58,237,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(124,58,237,0.06) 1px, transparent 1px)', backgroundSize: '80px 80px' }} />
      {/* Glow */}
      <div className="absolute" style={{ width: 800, height: 800, borderRadius: '50%', background: 'radial-gradient(circle, rgba(124,58,237,0.2) 0%, transparent 70%)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
