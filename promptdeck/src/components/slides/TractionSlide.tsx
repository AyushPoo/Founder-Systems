...
interface Props { headline?: string; metrics?: Metric[]; layout?: string; slideIndex: number; deckStyle?: string }
      ...
      <div className="flex-1 grid grid-cols-2 grid-rows-2" style={{ marginTop: 24 }}>
        {items.map((m, i) => (
          <div key={i} className="flex flex-col items-start justify-center px-20 py-6 relative"
            ...
            <div className="font-medium mb-3" style={{ fontSize: 18, color: t.textSub, letterSpacing: '0.01em' }}>{m.label}</div>
            {m.growth && (
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative">
        ...
        <div className="relative text-center">
          <div className="font-display font-black leading-none"
            style={{ fontSize: heroFs, letterSpacing: '-6px',
              fontFamily: "'Bricolage Grotesque', sans-serif", lineHeight: 0.9, color: t.text }}>
            {hero?.value || '$1.2M'}
          </div>
...
function TractionTimeline({ headline, metrics, up, t }: any) {
  const items: Metric[] = metrics?.length ? metrics.slice(0, 4) : DEFAULT_METRICS
  return (
    <div className="w-full h-full flex flex-col" style={{ background: t.bg }}>
      <div className="px-20 pt-14 shrink-0">
        <div className="text-xs font-semibold tracking-[0.3em] uppercase mb-2.5" style={{ color: GREEN }}>Traction</div>
        <EditableText value={headline || "Our growth story"} onChange={up('headline')} tag="h2"
          className="font-display font-black"
          style={{ fontSize: 46, letterSpacing: '-2px', fontFamily: "'Bricolage Grotesque', sans-serif", color: t.text }} />
      </div>
      <div className="flex-1 flex items-center px-16 relative">
        <div className="absolute left-16 right-16" style={{ height: 1, background: t.border, top: '50%' }} />
        {items.map((m, i) => {
          ...
          return (
            <div key={i} className="absolute flex flex-col items-center" style={{ left: positions[i], transform: 'translateX(-50%)' }}>
              <div className="text-center mb-6">
                <div className="font-display font-black leading-none"
                  style={{ fontSize: 44, letterSpacing: '-2px',
                    fontFamily: "'Bricolage Grotesque', sans-serif", color: t.text }}>
                  {m.value}
                </div>
                <div className="mt-1.5" style={{ fontSize: 15, color: t.textSub }}>{m.label}</div>
              </div>
              <div className="relative flex items-center justify-center" style={{ width: 20, height: 20 }}>
                <div className="absolute rounded-full" style={{ width: 36, height: 36, background: greenBg, border: `1px solid ${greenBorder}` }} />
                <div className="relative rounded-full z-10" style={{ width: 12, height: 12, background: GREEN }} />
              </div>
              {m.growth && (
...
export function TractionSlide({ headline, metrics = [], layout = 'grid', slideIndex, deckStyle }: Props) {
  ...
  const shared = { headline, metrics, up, t }
