...
// Red accent — always high-contrast regardless of theme
const RED = '#EF4444'
    ...
    <div className="w-full h-full flex flex-col" style={{ background: t.bg }}>
      <div className="px-20 pt-14 shrink-0">
        ...
        <EditableText value={headline || 'The scale of the problem'} onChange={up('headline')} tag="h2"
          className="font-display font-black" style={{ fontSize: 42, letterSpacing: '-1.5px',
            fontFamily: "'Bricolage Grotesque', sans-serif", color: t.text }} />
      </div>
...
function ProblemFullscreen({ headline, pain_points, up, t }: any) {
  const points = pain_points?.length ? pain_points : [
  ...
  return (
    <div className="w-full h-full flex" style={{ background: t.bg }}>
      <div className="shrink-0" style={{ width: 5, background: RED }} />
      <div className="flex-1 flex flex-col justify-between px-20 py-16">
        <div className="text-xs font-semibold tracking-[0.35em] uppercase" style={{ color: RED }}>The Problem</div>
        <div className="flex-1 flex items-center" style={{ maxWidth: '90%' }}>
          <EditableText value={hl} onChange={up('headline')} tag="h1" className="font-display font-black"
            style={{ fontSize: fs, letterSpacing: '-3px', lineHeight: 1.0,
              fontFamily: "'Bricolage Grotesque', sans-serif", color: t.text }} />
        </div>
        <div>
          <div className="w-16 h-0.5 mb-8" style={{ background: RED }} />
          <div className="flex items-center gap-6 flex-wrap">
            {points.slice(0, 3).map((p: PainPoint, i: number) => (
              <div key={i} className="flex items-center gap-3 px-5 py-2.5 rounded-full"
                ...
                <span style={{ fontSize: 18, color: t.textSub }}>{p.text}</span>
              </div>
            ))}
          </div>
        </div>
...
export function ProblemSlide({ headline, pain_points = [], stat, imageUrl, layout = 'statement', slideIndex, deckStyle }: Props) {
