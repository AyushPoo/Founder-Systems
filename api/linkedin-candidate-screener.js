  ...
  if (request.jobDescription === '__summarize_only__' || requestBody?.jobDescription === '__summarize_only__') {
    ...
    try {
      const modelResult = await invokeFounderJsonModel({
        ...
        systemPrompt: 'Create a CRISP candidate card. Think of it like a baseball card — key stats, not a biography.\n\nReturn ONLY JSON:\n{"ok":true,"domain":"sector","seniority":"Entry/Mid/Senior/Founder","tagline":"Role | Background | Key Cred (one line, pipe separated)","background":"2 sentences MAX. What they are doing now → where they came from. No fluff.","goodFor":"2-3 specific roles or contexts this person would be a good fit for","education":"One line: key qualifications comma separated","topSkills":["max 5 skills"]}\n\nRules:\n- tagline: MAX 60 chars. Format: Current Thing | Past Thing | Credential. E.g. "Founder (GradeSense) | Ex-Grant Thornton | CFA L1"\n- background: ONLY 2 sentences. First sentence = what they do NOW. Second = where they came from.\n- goodFor: What would you hire them for? Be specific.\n- If they are a FOUNDER, domain should reflect their startup not their past career\n- Do NOT list every job. Summarize the trajectory.\n- Do NOT say "driven by" or "committed to" or "passionate about"',
        userPrompt: 'Candidate card. Be EXTREMELY concise. Tagline + 2 sentence background + what they are good for.\n\n' + profileText.slice(0, 4000),
        maxOutputTokens: 350,
        modelTier: 'quality',
