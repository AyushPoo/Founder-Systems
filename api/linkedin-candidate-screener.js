  ...
  if (request.jobDescription === '__summarize_only__' || requestBody?.jobDescription === '__summarize_only__') {
    ...
    try {
      const modelResult = await invokeFounderJsonModel({
        ...
        systemPrompt: 'You are creating a DECISION-READY candidate snapshot for a recruiter or investor. NOT a reformatted LinkedIn profile. Think: what would a hiring manager need to know in 10 seconds?\n\nRules:\n- candidateSummary: 2-3 tight sentences. Lead with what they are NOW (if they are building something, say that first). Then trajectory. Then what makes them notable. Never use "keen on" or "passionate about".\n- experience: MAX 3 bullet points. Only significant roles. Format: "Role at Company — one line on what they did (dates)". If they are currently a FOUNDER/BUILDER, that goes FIRST.\n- education: MAX 2 items. Professional certs (CA/CFA/CPA) and highest degree only. Format: "Qualification — Status/Year"\n- skills: Top 5 most relevant technical skills only\n- fitSignals: 2-3 things that actually differentiate them (Big 4 pedigree, founding experience, rare combo of skills, etc.)\n\nIMPORTANT: If profile shows they are building/founded a company, that is the MOST IMPORTANT thing — put it first in summary and experience.\n\nReturn ONLY valid JSON: {"ok":true,"domain":"sector","seniority":"level","candidateSummary":"...","experience":["..."],"education":["..."],"skills":["..."],"fitSignals":["..."]}',
        userPrompt: 'Decision-ready snapshot. What is this person doing NOW? What is their strongest signal? Be crisp.\n\n' + profileText.slice(0, 4000),
        maxOutputTokens: 500,
        modelTier: 'quality',
